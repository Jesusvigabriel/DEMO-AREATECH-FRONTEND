    ...
    >
      <v-list>
        ...
        <v-list-group
          ...
          no-action
          @click:activator="expandirSoloGrupo(i)"
        >
          <template #activator>
...
export default {
  ...
  methods: {
    ...
    cargarYAgruparMenus() {
      ...
      roles.getAllMenuUser(this.usuarioId)
        .then(menus => {
          ...
          // SOLO LOS GRUPOS QUE TIENEN ITEMS PARA EL DRAWER MOBILE
          const todosGrupos = [
    ...
    expandirSoloGrupo(index) {
      this.gruposMobile.forEach((g, i) => {
        g.expanded = i === index ? !g.expanded : false
      })
    }
  },
  mounted() {
    ...
    // Los menús los maneja el watcher
  }
}
</script>
